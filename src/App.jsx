...
const css = `
  ...
  * { box-sizing: border-box; }
  ...
  .sarro-fade::before { left: 0; background: linear-gradient(90deg, #130E0B 0%, transparent 100%); }
  .sarro-fade::after { right: 0; background: linear-gradient(270deg, #130E0B 0%, transparent 100%); }
  ...
  .pizza-hero-grid {
    ...
    padding: clamp(38px, 6vw, 76px) 0 clamp(48px, 7vw, 84px);
  }
  .pizza-hero-copy { position: relative; z-index: 1; }
  .pizza-eyebrow {
    display: inline-flex;
    align-items: center;
    gap: 9px;
  ...
  .pizza-hero-visual img { width: 100%; height: 100%; min-height: inherit; object-fit: cover; display: block; }
  .pizza-hero-image-shade { position: absolute; inset: 0; background: linear-gradient(180deg, transparent 50%, rgba(19,14,11,.72)); pointer-events: none; }
  .pizza-hero-stamp {
    position: absolute;
    left: 18px;
    bottom: 18px;
  ...
  @media (max-width: 720px) {
    .pizza-hero-grid { grid-template-columns: 1fr; gap: 24px; padding: 42px 0 42px; }
    .pizza-hero-copy { padding: 0 2px; }
    .pizza-hero-visual { min-height: 250px; border-radius: 14px 14px 72px 14px; }
    .pizza-hero-visual img { min-height: 250px; }
    .pizza-hero-stamp { left: 12px; bottom: 12px; padding: 9px 12px; }
  }
  ...
  return (
    <img
      ...
      style={{
        ...
        height: size,
        objectFit: "contain",
        filter: glow ? `drop-shadow(0 0 0 ${C.black}) drop-shadow(0 8px 18px ${C.orange}80)` : undefined,
        ...style,
      }}
    />
  );
...
function Btn({ children, onClick, variant = "primary", full, disabled, small, style = {} }) {
  const base = {
    primary: { background: `linear-gradient(100deg, ${C.orange}, #8F251F)`, color: C.white },
...
function printHTML(body) {
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>Mil Grau Pizzaria</title><style>
...
function printReceipt(o, settings = {}) {
  ...
  `).join("");
  printHTML(`
    <h1>${esc(settings.storeName || "MIL GRAU PIZZARIA")}</h1>
    <div class="sub">Delivery e retirada<br />${settings.address || ""}</div>
    <hr />
    ...
    <hr />
    <div class="c">Obrigado! Volta sempre 🔥<br />pizzaria · cardápio digital</div>
  `);
}
...
function printCashSummaryReceipt(reg, settings = {}) {
  ...
  printHTML(`
    <h1>${esc(settings.storeName || "MIL GRAU PIZZARIA")}</h1>
    <div class="sub">RESUMO DE CAIXA / PDV</div>
    <hr />
    ...
    <hr />
    <div class="c">${esc(settings.storeName || "Mil Grau Pizzaria")} · Frente de Caixa PDV</div>
  `);
}
...
function printDriverSettlementReceipt(settlement, settings = {}) {
  ...
  printHTML(`
    ...
    <div style="margin-top: 25px; text-align: center; font-size: 11px;">
      ____________________________________<br />
      Assinatura da Mil Grau Pizzaria
    </div>
    <hr />
    <div class="c">${esc(settings.storeName || "Mil Grau Pizzaria")} · Controle de Entregas</div>
  `);
}

function buildGoogleMapsMultiStopUrl(orders, storeAddress = "Endereço da pizzaria") {
...
function Hero({ store, onOrder }) {
  const eta = store.settings?.eta || "Prazo a definir";

  return (
    <section className="pizza-hero relative overflow-hidden" style={{ background: C.black }}>
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ background: `radial-gradient(75% 80% at 88% 12%, ${C.orange}35, transparent 65%), radial-gradient(55% 90% at 0% 100%, ${C.yellow}12, transparent 72%)` }}
      />
      <div className="relative px-4 sm:px-6 max-w-[1120px] mx-auto w-full">
        <header className="flex items-center justify-between gap-3 pt-4 sm:pt-6">
          <div className="flex items-center min-w-0">
            <Logo size={88} glow withText style={{ width: "clamp(104px, 29vw, 126px)", height: "auto", maxWidth: "42vw" }} />
          </div>
        ...
        <div className="pizza-hero-grid">
          <div className="pizza-hero-copy">
            <span className="pizza-eyebrow">CARDÁPIO DE DEMONSTRAÇÃO</span>
            <h1 style={{ fontFamily: font.display, color: C.white, fontSize: "clamp(40px, 7vw, 76px)", lineHeight: .98, letterSpacing: "-.035em", margin: "20px 0 14px", fontWeight: 400 }}>
              HOJE É DIA DE<br />
              <span style={{ color: C.yellowLight, fontStyle: "italic" }}>FATIA BOA.</span>
            </h1>
            <p style={{ color: "#C7B9AE", fontSize: "clamp(14px, 2vw, 16px)", lineHeight: 1.7, maxWidth: 460, margin: 0 }}>
              Pizza quentinha, feita para dividir — ou não. Escolha a sua e deixe o forno com o resto.
            </p>
            <div className="flex flex-wrap items-center gap-3 mt-6">
              <Btn onClick={onOrder} style={{ padding: "13px 22px", borderRadius: 999, color: C.white, boxShadow: `0 12px 30px ${C.orange}3b` }}>
                EXPLORAR O CARDÁPIO <span aria-hidden="true" style={{ marginLeft: 8 }}>↗</span>
              </Btn>
              <span style={{ color: "#BCA99B", fontSize: 12, fontWeight: 700 }}>🛵 {eta} · delivery e retirada</span>
            </div>
            <div className="flex items-center gap-2 mt-5" style={{ color: "#907E71", fontSize: 10, letterSpacing: ".04em" }}>
              <span style={{ display: "inline-block", width: 20, height: 1, background: C.yellow }} />
              Itens, valores, taxas e prazos são demonstrativos e serão atualizados com as informações da loja.
            </div>
          </div>

          <div className="pizza-hero-visual">
            <img src="/img/pizzeria-hero.jpg" alt="Pizza artesanal recém-saída do forno" />
            <div className="pizza-hero-image-shade" />
            <div className="pizza-hero-stamp">
              <span style={{ fontSize: 9, letterSpacing: ".18em", color: C.yellowLight, fontWeight: 900 }}>FEITO PARA</span>
              <span style={{ fontFamily: font.display, fontSize: 22, lineHeight: 1.05, color: C.white }}>compartilhar.</span>
            </div>
          </div>
        </div>
      </div>
            ...
            {menuCategories.map((c) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: on ? `linear-gradient(100deg, ${C.orange}, #8F251F)` : C.gray850,
                    color: on ? C.white : "#c9c9c9",
  ...
  return (
    <div className="pb-6">
      ...
      <div className="px-4 pt-7">
        <Card className="p-5" style={{ background: `linear-gradient(120deg, ${C.orange}26, ${C.gray850})`, borderColor: `${C.yellow}35` }}>
          <div style={{ color: C.yellowLight, fontSize: 10, fontWeight: 900, letterSpacing: ".16em" }}>UM BOM MOTIVO PARA REUNIR TODO MUNDO</div>
          <div style={{ color: C.white, fontFamily: font.display, fontSize: 22, marginTop: 5 }}>A mesa fica melhor com pizza.</div>
          <div style={{ color: "#c9c9c9", fontSize: 12.5, marginTop: 4 }}>Escolha seus sabores favoritos e encontre o próximo pedido da casa.</div>
        </Card>
      </div>
    </div>
  );
...
function CartScreen({ store, goCheckout, onOpen }) {
  ...
  const upsell = store.products
    .filter((p) => p.available && ["sobremesas", "bebidas", "combos-pizzas", "sucos"].includes(p.cat))
  ...
  if (!cart.length) {
    return (
      <div className="px-4 py-16 text-center">
        ...
        <p style={{ color: "#8a8a8a", fontSize: 13, marginTop: 8 }}>Escolha sua pizza favorita e a gente cuida do resto.</p>
...
const SYNONYMS = {
  calabresa: ["linguiça"], queijo: ["mozzarella", "catupiry", "provolone"],
  doce: ["chocolate", "morango"], borda: ["catupiry", "cheddar"],
  vegetariana: ["margherita", "vegetariana"], barato: [],
};
...
function Checkout({ store, totals, onBack, onDone }) {
  ...
  const [f, setF] = useState({
    ...
    street: "", number: "", district: "", city: "", ref: "",
    ...
    <div className="px-4 py-5 pb-6">
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <Field label="WhatsApp" value={f.phone} onChange={(v) => set("phone", v)} ph="(00) 00000-0000" />
      ...
      {step === 2 && (
        <div className="space-y-3">
          ...
          <Choice on={f.type === "delivery"} onClick={() => set("type", "delivery")} icon="🛵"
            title="Delivery" sub={`${store.settings?.eta || "Prazo a definir"} · taxa ${brl(store.fee)}`} />
          <Choice on={f.type === "pickup"} onClick={() => set("type", "pickup")} icon="🏪"
            title="Retirar na loja" sub={`${store.settings?.eta || "Prazo a definir"} · sem taxa`} />
        </div>
      )}
        ...
        f.type === "pickup" ? (
          <div>
            ...
            <Card className="p-4 mt-4">
              <div style={{ color: C.white, fontWeight: 800, fontSize: 14 }}>{store.settings?.storeName || "Mil Grau Pizzaria"}</div>
              <div style={{ color: "#9a9a9a", fontSize: 12.5, marginTop: 6, lineHeight: 1.5 }}>
                {store.settings?.address || "Endereço a cadastrar nas configurações"}<br />
                {store.settings?.hours || "Horário a definir"}
              </div>
            </Card>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Bairro" value={f.district} onChange={(v) => set("district", v)} ph="Seu bairro" />
            </div>
            <Field label="Cidade" value={f.city} onChange={(v) => set("city", v)} ph="Sua cidade" />
...
const TRACK_STEPS = [
  ...
  { key: "PREPARO", label: "Na forno agora", icon: "🔥" },
  ...
  return (
    <div className="px-4 py-5 pb-6">
      <div
        ...
        style={{ background: `linear-gradient(130deg, ${C.orange}, #8F251F)`, color: C.white }}
      >
        <div style={{ fontSize: 11, fontWeight: 800, opacity: 0.75 }}>Pedido #{order.code}</div>
          <div style={{ fontFamily: font.display, fontStyle: "italic", fontSize: 26, lineHeight: 1.02, marginTop: 4 }}>
            {done ? "SUA PIZZA CHEGOU! 🍕" : order.type === "pickup" ? "SUA PIZZA ESTÁ SAINDO!" : "SUA PIZZA ESTÁ A CAMINHO!"}
        </div>
        <div style={{ fontSize: 12.5, fontWeight: 700, marginTop: 8 }}>
          {done ? "Bom apetite. Volta sempre!" : order.type === "pickup" ? "Pronto para retirada" : `Previsão: ${store.settings?.eta || "prazo a definir"}`}
        </div>
      </div>
  ...
  if (!me) {
    return (
      <div className="px-4 py-16 text-center max-w-[640px] mx-auto">
        ...
        <p style={{ color: "#8a8a8a", fontSize: 13, marginTop: 6 }}>Faça seu primeiro pedido para entrar no Clube da Casa.</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-3 sm:px-4 py-5 pb-6 max-w-[720px] mx-auto w-full overflow-x-hidden">
      ...
      <Card className="p-4" style={{ background: `linear-gradient(120deg, ${C.orange}26, ${C.gray850})`, borderColor: `${C.orange}44` }}>
        <div style={{ color: C.white, fontWeight: 900, fontSize: 14 }}>🍕 Clube da Casa</div>
        <div style={{ color: "#c0c0c0", fontSize: 12, marginTop: 4 }}>
          A cada R$ 10 gastos você ganha 1 ponto. Com 100 pontos, uma pizza é por nossa conta.
        </div>
      ...
      {!checkout && store.tab !== "carrinho" && cartCount > 0 && (
        <button
          ...
          style={{
            ...
            background: `linear-gradient(100deg, ${C.orange}, #8F251F)`, color: C.white,
...
function ProductForm({ initial, store, onClose }) {
  const [f, setF] = useState(() => (initial ? {
    ...
    time: String(initial.time || ""), stock: String(initial.stock ?? ""),
    badges: initial.badges || [], groups: initial.groups || [],
    available: initial.available, builder: initial.builder,
  } : {
    name: "", cat: "pizzas-grandes", emoji: "🍕", description: "", ingredients: "",
    price: "", promoOn: false, promo: "", time: "", stock: "",
  ...
  const save = async () => {
    ...
    try {
      await store.saveProduct(initial?.id, {
        ...
        time: f.time.trim() ? Math.max(1, parseInt(f.time) || 1) : 0,
      ...
      >
        ...
        <div className="p-4 space-y-4">
            ...
            <div className="flex-1 space-y-3">
              <label className="block">
                ...
                <input value={f.name} onChange={(e) => set("name", e.target.value)}
                  ...
                  placeholder="Ex.: Pizza vegetariana" />
              </label>
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <label className="block">
              <span style={{ color: "#9a9a9a", fontSize: 11, fontWeight: 700 }}>Preparo (min, opcional)</span>
              <input value={f.time} onChange={(e) => set("time", e.target.value)} inputMode="numeric"
                className="w-full rounded-xl px-3 py-2.5 mt-1 outline-none" style={inField} placeholder="Não informado" />
            </label>
            <label className="block">
          ...
          <Card key={p.id} className="p-3" style={{ opacity: p.available ? 1 : 0.55 }}>
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div style={{ color: "#7a7a7a", fontSize: 11 }}>
                  {store.categories.find((c) => c.id === p.cat)?.label || "Sem categoria"} · {p.time > 0 ? `${p.time} min` : "tempo não informado"} · {p.stock != null ? `estoque ${p.stock}` : "estoque não informado"}
                </div>
                <div className="flex items-center gap-2">
                  <span style={{ color: C.yellowLight, fontWeight: 900, fontSize: 13.5, marginTop: 2 }}>
                    {p.price > 0 ? brl(p.promo || p.price) : "Preço a confirmar"}
                  </span>
                  {p.promo && <span style={{ color: "#6e6e6e", fontSize: 10.5, textDecoration: "line-through" }}>{brl(p.price)}</span>}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between mt-3 pt-3" style={{ borderTop: `1px solid ${C.gray800}` }}>
              ...
              <button
                ...
                disabled={p.price <= 0 && !p.available}
                title={p.price <= 0 && !p.available ? "Informe o preço antes de disponibilizar" : undefined}
...
function AdminFinance({ store, now }) {
  ...
  const exportCSV = () => {
    downloadCSV(`financeiro-pizzaria-${range}.csv`, [
      ["MIL GRAU PIZZARIA — Financeiro"],
...
function AdminReports({ store, now }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <ReportCard title="Vendas por dia" cols={["Dia", "Pedidos", "Faturamento", "Ticket médio"]} rows={salesRows} csvName="pizzaria-vendas.csv" />
      <ReportCard title="Produtos vendidos" cols={["Produto", "Qtd", "Receita"]} rows={productRows} csvName="pizzaria-produtos.csv" />
      <div className="grid lg:grid-cols-2 gap-3">
        <ReportCard title="Formas de pagamento" cols={["Pagamento", "Pedidos", "Total"]} rows={payRows} csvName="pizzaria-pagamentos.csv" />
        <ReportCard title="Canais de venda" cols={["Canal", "Pedidos", "Total"]} rows={channelRows} csvName="pizzaria-canais.csv" />
        <ReportCard title="Entregadores" cols={["Entregador", "Veículo", "Entregas", "Valor entregue"]} rows={driverRows} csvName="pizzaria-entregadores.csv" />
        <ReportCard title="Top clientes" cols={["Cliente", "WhatsApp", "Pedidos", "Gasto", "Classe"]} rows={customerRows} csvName="pizzaria-clientes.csv" />
      </div>
      <ReportCard title="Cancelamentos" cols={["Pedido", "Cliente", "Valor", "Quando", "Canal"]} rows={canceledRows} csvName="pizzaria-cancelamentos.csv" />
    </div>
  );
}
...
function CouponForm({ initial, onClose, onSaved }) {
  ...
  return (
    <FormShell title={initial ? "EDITAR CUPOM" : "NOVO CUPOM"} sub={initial?.code} onClose={onClose}>
      {!initial && <Field label="Código (sem espaços)" value={f.code} onChange={(v) => set("code", v.toUpperCase())} ph="EX: PIZZA10" />}
...
function PromoForm({ initial, onClose, onSaved }) {
  ...
  return (
    <FormShell title={initial ? "EDITAR PROMOÇÃO" : "NOVA PROMOÇÃO"} onClose={onClose}>
      <Field label="Nome" value={f.name} onChange={(v) => set("name", v)} ph="Ex: Promoção da casa" />
...
function UserForm({ initial, roles, drivers, onClose, onSaved }) {
  ...
  return (
    <FormShell title={initial ? "EDITAR USUÁRIO" : "NOVO USUÁRIO"} sub={initial?.username} onClose={onClose}>
      <Field label="Nome" value={f.name} onChange={(v) => set("name", v)} ph="Ex: Maria Oliveira" />
      ...
      <div className="mt-3 space-y-2.5">
        ...
        <label className="block">
          ...
          <input
            ...
            placeholder="ex.: https://pedidos.pizzaria.com.br"
            className="w-full rounded-lg px-2.5 py-2 mt-1 outline-none"
            style={inField}
          />
...
function AdminIntegrations({ store }) {
  ...
  const [wa, setWa] = useState({ phone_number_id: "", token: "", verify: "", template: "pizzaria_status" });
      ...
      <div className="grid lg:grid-cols-2 gap-3">
        ...
        <Card className="p-4">
          ...
          <p style={{ color: "#8a8a8a", fontSize: 11.5, marginTop: 6, lineHeight: 1.5 }}>
            Mensagens automáticas a cada status do pedido (recebido, pagamento, forno, pronto, saiu, entregue).
            Requer um app no <span style={{ color: "#c0c0c0" }}>developers.facebook.com</span> com template aprovado
            ({w.template || "pizzaria_status"}, 1 parâmetro de corpo).
          </p>
          <div className="mt-3 space-y-2">
...
function Splash({ error, onRetry }) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: C.black }}>
      <div style={{ animation: error ? "none" : "sarrofloat 1.6s ease-in-out infinite" }}>
        <Logo size={110} glow withText />
      </div>
      <div style={{ fontFamily: font.display, fontStyle: "italic", color: C.orange, fontSize: 24, marginTop: 22 }}>
        {error ? "O FORNO APAGOU!" : "ACENDENDO O FORNO…"}
      </div>
          ...
          {ROLES.map((r) => {
            ...
            return (
              <a
                ...
                style={{
                  background: role === r.id ? `linear-gradient(100deg, ${C.orange}, #8F251F)` : "transparent",
                  color: role === r.id ? C.white : "#8a8a8a",
