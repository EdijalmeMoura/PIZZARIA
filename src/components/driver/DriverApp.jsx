...
export default function DriverApp({ store, now, driverOverride = null, preview = false }) {
  ...
  const brandName = store.settings?.storeName || "Mil Grau Pizzaria";
  ...
  const handleArrived = (o) => {
    ...
    const msg = `Olá, ${o.customer?.name || "cliente"}! 🛵 Sou o entregador da ${brandName}. Já cheguei no seu endereço (${o.customer?.addr || ""}) com o pedido #${o.code}. Estou no portão/portaria te aguardando! 🔥`;
  ...
  const confirmDelivery = (o, thankYou) => {
    ...
    if (thankYou) {
      const msg = `Olá, ${o.customer?.name}! Seu pedido #${o.code} da ${brandName} foi entregue. Bom apetite e muito obrigado pela preferência! Se puder nos avaliar com 5 estrelas, ficaremos gratos! ⭐⭐⭐⭐⭐`;
      openWhatsApp(o.customer?.phone, msg);
    }
            ...
            {waiting.map((o) => (
              <Card key={o.id} className="p-3.5">
                ...
                <div className="mt-2 flex gap-2">
                  ...
                  <Btn small variant="dark" onClick={() => openWhatsApp(o.customer?.phone, `Olá, ${o.customer?.name}! Sou o entregador da ${brandName}, sobre o pedido #${o.code}.`)}>
                    💬 WhatsApp
                  </Btn>
                </div>
              </Card>
                ...
                <div className="flex justify-between items-center gap-2 py-1.5" style={{ borderBottom: `1px solid ${C.gray850}` }}>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {!done && (
                      <>
                        <button
                          onClick={() => openWhatsApp(o.customer?.phone, `Olá, ${o.customer?.name}! Sou o entregador da ${brandName}, sobre o pedido #${o.code}.`)}
            ...
            <div className="space-y-2.5">
              <div className="p-3 rounded-xl" style={{ background: C.gray850, border: `1px solid ${C.gray800}` }}>
                ...
                <div className="flex gap-2 mt-2">
                  <button
                    onClick={() => openWhatsApp(problemModal.customer?.phone, `Olá, ${problemModal.customer?.name}! Sou o entregador da ${brandName} com seu pedido #${problemModal.code}. Já estou no portão/portaria mas não consegui contato. Me responde aqui, por favor! 🛵`)}
              ...
              <div className="p-3 rounded-xl" style={{ background: C.gray850, border: `1px solid ${C.gray800}` }}>
                ...
                <button
                  onClick={() => openWhatsApp(problemModal.customer?.phone, `Olá, ${problemModal.customer?.name}! Sou o entregador da ${brandName} com o pedido #${problemModal.code}. Estou na sua rua mas não localizei o endereço (${problemModal.customer?.addr}). Pode me mandar a localização em tempo real ou um ponto de referência?`)}
