        ...
        <div className="p-4 space-y-4">
          <div className="flex gap-4">
            ...
            <div className="flex-1 space-y-3">
              <Field label="Nome *" value={f.name} onChange={(v) => set("name", v)} ph="Ex.: Pizza de calabresa" />
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button type="button" onClick={() => set("builder", !f.builder)} className="rounded-xl px-3 py-2 font-bold" style={{ background: f.builder ? `${C.orange}1e` : C.gray850, border: `1px solid ${f.builder ? C.orange : C.gray800}`, color: f.builder ? C.orange : "#9a9a9a", fontSize: 12 }}>🛠️ Monte sua pizza {f.builder ? "✓" : ""}</button>
          </div>
...
export function PromoForm({ initial, onClose, onSaved }) {
  ...
  return (
    <FormShell title={initial ? "EDITAR PROMOÇÃO" : "NOVA PROMOÇÃO"} onClose={onClose}>
      <Field label="Nome" value={f.name} onChange={(v) => set("name", v)} ph="Ex: Noite da Pizza" />
