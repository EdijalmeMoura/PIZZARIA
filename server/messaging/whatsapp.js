// ============================================================
// MIL GRAU PIZZARIA — WhatsApp Cloud API (Meta)
//
...
export function waCredentials(settingsLike) {
  return {
    ...
    template: settingsLike.wa_template || "pizzaria_status",
  };
}
...
export function buildOrderMessage(event, order, extra = {}) {
  const head = `🔥 ${(extra.storeName || "Mil Grau Pizzaria").toUpperCase()}`;
  const code = `#${order.code}`;
  const items = itemsList(order);

  switch (event) {
    case "recebido":
      return `${head}\n\nPedido ${code} recebido! 🎉\n\n${items}\n\n💰 Total: ${brl_(order.total)}\n\n⏳ Aguardando confirmação do pagamento. Assim que cair, o forno acende!`;
    case "pagamento_ok":
      return `${head}\n\n✅ Pagamento confirmado!\n\nPedido ${code} entrou na fila da cozinha.\n\n${items}\n\n💰 Total: ${brl_(order.total)}\n\nPrevisão: ${extra.eta || "prazo a confirmar"} 🛵`;
    case "preparo":
      return `${head}\n\n🔥 SEU PEDIDO ESTÁ NO FORNO!\n\nPedido ${code} em preparo agora. Capricho total saindo da cozinha!`;
    case "pronto":
      return order.type === "pickup"
        ? `${head}\n\n🔥 SEU PEDIDO ${code} ESTÁ PRONTO!\n\nPode vir buscar na loja: ${extra.address || "endereço informado no pedido"} 🏪`
    ...
    case "entregue":
      return `${head}\n\n✅ ENTREGUE!\n\nPedido ${code} chegou. Bom apetite e obrigado por escolher a gente! 🍕`;
    case "teste":
