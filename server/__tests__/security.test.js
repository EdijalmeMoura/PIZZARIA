...
describe("Security - zod validation", () => {
  ...
  it("rejeita produto com preço negativo", () => {
    expect(() => validateProduct({ name: "Pizza Calabresa", cat: "pizzas", price: -5 })).toThrow();
  });

  it("aceita produto válido", () => {
    const p = validateProduct({ name: "Pizza Margherita", cat: "pizzas", price: 25.9 });
    expect(p.name).toBe("Pizza Margherita");
