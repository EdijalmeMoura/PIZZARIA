// ============================================================
// PIZZARIA — API REST + WebSocket (servidor único)
//
...
function fireWhatsApp(order, event, extra = {}) {
  try {
    ...
    const text = buildOrderMessage(event, order, {
      storeName: st.store_name || "Mil Grau Pizzaria",
      ...extra,
    });
    if (!to) {
...
server.on("upgrade", (req, socket, head) => {
  ...
  if (pathname === "/ws") {
    wss.handleUpgrade(req, socket, head, (ws, wreq) => {
      ...
      const m = /(?:^|;\s*)pizzeria_session=([^;]+)/.exec(wreq.headers.cookie || "");
      ws.isStaff = false;
      if (m) {
        const u = db
...
app.post("/api/orders/external", requireRole("ADMIN", "GERENTE", "ATENDIMENTO", "EXPEDICAO"), (req, res) => {
  ...
  `).run(id, code, channel, Date.now(), name,
    ...
    "Rua Projetada, 100 — endereço informado no pedido",
...
app.post("/api/routes/optimize", requireRole("ADMIN", "GERENTE", "ATENDIMENTO", "EXPEDICAO"), async (req, res) => {
  ...
  try {
    ...
    const settings = getSettings();
    const storeAddress = settings.address || "Endereço da pizzaria";
...
app.post("/api/orders/:id/pay", async (req, res) => {
  ...
  const ps = getPaymentSettings();
  const settings = getSettings();
  const pixKey = ps.pixKey || settings.pixKey || "";
  const pixCode = generatePixBRCode({
    key: pixKey,
    name: settings.storeName || "Mil Grau Pizzaria",
    city: "BRASIL",
    amount: o.total,
    txid: `PED${o.code}`,
  });
  ...
  if (ps.payHandle) {
    ...
    const items = [];
    if (o.discount > 0) {
      items.push({ quantity: 1, price: cents(o.total), description: "Pedido #" + o.code + " — Mil Grau" });
    } else {
...
app.get("/api/integrations/overview", requireRole("ADMIN", "GERENTE"), (_req, res) => {
  ...
  res.json({
    whatsapp: {
      ...
      template: st.wa_template || "pizzaria_status",
...
async function printToThermal(kind, orderId) {
  ...
  const buffer =
    kind === "kitchen" ? escpos.buildKitchenComanda(order, st.store_name || "MIL GRAU PIZZARIA") :
    kind === "expedition" ? escpos.buildExpeditionComanda(order, st.store_name || "MIL GRAU PIZZARIA") :
...
function autoPrintKitchen(order) {
  ...
  escpos.sendRaw(st.printer_host, st.printer_port || "9100", escpos.buildKitchenComanda(order, st.store_name || "MIL GRAU PIZZARIA"))
...
app.post("/api/cash/print-summary", requireRole("ADMIN", "GERENTE", "ATENDIMENTO"), async (req, res) => {
  ...
  if (st.printer_enabled === "1" && st.printer_host) {
    try {
      ...
      const lines = [
        "================================",
        "  MIL GRAU PIZZARIA - RESUMO DE CAIXA ",
        "================================",
...
if (hasFrontend) {
  ...
  app.get(/^(?!\/api|\/ws|\/healthz).*/, (_req, res) => {
    res.status(503).type("html").send(
      "<!doctype html><meta charset=utf-8><title>MIL GRAU PIZZARIA</title>" +
...
server.listen(PORT, HOST, () => {
  console.log(`[api] MIL GRAU backend em http://${HOST}:${PORT}`);
